...
L:RegisterTranslations("enUS", function()
	return {
		...
		polymorphyouend_trigger = "Greater Polymorph" .. BigWigs.AURAREMOVEDSELF_SUFFIX,
...
function module:Events(msg)
	...
	if self.db.profile.mc then
		...
		elseif mcother then
			...
			self:SetCandyBarOnClick("BigWigsBar " .. string.format(L["mindcontrol_bar"], mcother), function(name, button, extra)
				TargetByName(extra, true)
			end, mcother)
...
function module:BigWigs_RecvSync(sync, rest, nick)
	...
	elseif sync == syncName.orb then
		...
		if self.db.profile.orb then
			...
			self:SetCandyBarOnClick("BigWigsBar " .. string.format(L["orb_bar"], rest), function(name, button, extra)
				TargetByName(extra, true)
			end, rest)
		end
